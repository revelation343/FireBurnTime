...
public class FireBurnTime implements WurmServerMod, PreInitable, Configurable {
    ...
    public void preInit() {
        ...
        try {
                    ...
                    if (methodCall.getMethodName().equals("sendEnchantmentStrings")) {
                        ctMethod.insertAt(methodCall.getLineNumber(),
                                "{" +
                                        ...
                                        "    coolingSpeed *= (float)Math.pow(0.8999999761581421, this.getRarity());"+

                                        "float decreaseTemperature = coolingSpeed * Math.max(1.0f, 11.0f - Math.max(1.0f, 20.0f * Math.max(30.0f, target.getCurrentQualityLevel()) / 200.0f));"+
                                        "int secondsRemaining = Math.round(Math.max(0, target.getTemperature() - " + _TargetTemperature + ") / decreaseTemperature);"+

                                        "performer.getCommunicator().sendNormalServerMessage(\"It will be completely cold in about \" + secondsRemaining / 60 + \" minutes and \" + secondsRemaining % 60 + \" seconds.\");"+
                                        "}"
                        );
                    }
                }
            ...
            _Logger.severe("FireBurnTime 1.5 could not be applied.");
